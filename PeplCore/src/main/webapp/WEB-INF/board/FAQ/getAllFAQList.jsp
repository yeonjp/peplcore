...
<body>
	<div class="container">
		...
		<div class="main">
			<form action="insertFAQView.bc" method="post">
				<h1>FAQ전체리스트</h1>
				...
				<table border="1">
					<c:forEach items="${faqList}" var="f" varStatus="i">
						<tr>
							<th rowspan="2">${f.getBseq()}</th>
							<th>Q</th>
							<td><a href="getOneFAQList.bc?seq=${f.getBseq()}">${f.getBtitle()}</a>
							<!--  <input type="text" value="${f.getBtitle()}" readonly> -->
							</td>
							<td rowspan="2"><a href="updateFAQView.bc?seq=${f.getBseq()}">수정</a></td>
							<td rowspan="2"><a href="deleteFAQ.bc?seq=${f.getBseq()}">삭제</a></td>
						</tr>
						<tr>
							<th>A</th>
							<td>${f.getBcontent()}</td>
