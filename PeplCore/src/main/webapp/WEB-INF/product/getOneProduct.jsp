		...
		<div class="main">
			...
			<div class="maincontent">
				...
				<table>
					...
					<tr>
						<th>상품 분류</th>
						<td><c:if test="${product.pcategory == '상의' }">
								${product.getPcategory() }
							</c:if> <c:if test="${product.pcategory == '하의' }">
								${product.getPcategory() }
							</c:if> <c:if test="${product.pcategory == '신발' }">
								${product.getPcategory() }
							</c:if> <c:if test="${product.pcategory == '악세사리' }">
								${product.getPcategory() }
							</c:if>
						</td>
					...
					<tr>
						<th>상품 이름</th>
						<%-- <input type="text" name="pname" value="${product.getPname()}" readonly="readonly"/>  --%>
						<td>${product.getPname() }</td>
					</tr>
					<tr>
						<th>상품 수량</th>
						<%-- <input type="text" name="pnum" value="${product.getPnum()}" readonly }/> --%>
						<td>${product.getPnum() }</td>
					</tr>
					<tr>
						<th>상품 품절 여부</th>
						<td><c:if test="${product.poutStock == '품절' }">
							${product.getPoutStock() }
						</c:if> <c:if test="${product.poutStock == '재고있음' }">
							${product.getPoutStock() }
						</c:if></td>
						<!-- 						<td>
							<input type="radio" name="poutStock" value="품절">품절
							<input type="radio" name="poutStock" value="재고있음">재고있음
						</td> -->
					</tr>
					<tr>
						<th>상품 사이즈</th>
					...
					<tr>
						...
						<td><textarea name="pdescription" cols="60" rows="10"
								readonly>${product.getPdescription() }</textarea></td>
					</tr>
					<tr>
						<th>상품 원가</th>
				...
				<input type="submit" name="listProduct" class="listBtn" value="목록으로"
					onclick="location.href='productManagement.pc'"></input>
			</div>
			<!-- end of maincontent-->
		</div>
		<!--end of main-->
